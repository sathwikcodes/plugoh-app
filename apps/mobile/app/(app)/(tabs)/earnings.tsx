...
export default function EarningsScreen() {
  ...
  return (
    <Screen>
      <SectionTitle
        title="Earnings"
        subtitle="See held funds, released income, and your current creator tier."
      />
      <View style={{ flexDirection: 'row', gap: 12 }}>
        <Card style={{ flex: 1 }}>
          ...
          <Text style={{ ...theme.typography.title, color: theme.colors.foreground }}>
            ₹{Math.round(earnings.data?.total_earnings ?? 0).toLocaleString('en-IN')}
          </Text>
        </Card>
        <Card style={{ flex: 1 }}>
      ...
      {(earnings.data?.transactions ?? []).length === 0 ? (
        <EmptyState
          ...
          subtitle="Completed campaigns will appear here with release-ready financial history."
        />
      ) : (
        <FlashList
