...
export default function CampaignsScreen() {
  ...
  const visible = (campaigns.data ?? []).filter((item) => {
  ...
  return (
    <Screen>
      <SectionTitle title="Campaigns" subtitle="Track requests, funded work, delivery, and completion without losing context." />
      <TextInput
        value={search}
        ...
        {filters.map((filter) => (
          <Pressable
            ...
            style={{
              ...
              backgroundColor: activeFilter === filter.label ? theme.colors.accentSoft : theme.colors.surfaceWarm,
            }}
          >
            <Text style={{ ...theme.typography.label, color: activeFilter === filter.label ? theme.colors.accentStrong : theme.colors.muted }}>{filter.label}</Text>
          </Pressable>
        ))}
      </View>
      {visible.length === 0 ? (
        <EmptyState title="No campaigns in this view" subtitle="Change the status filter or wait for a new request to land." />
      ) : (
        <FlashList
          data={visible}
          renderItem={({ item }) => (
            <Pressable onPress={() => router.push(`/(app)/campaigns/${item.id}` as never)}>
              <Card>
                <View style={{ flexDirection: "row", justifyContent: "space-between", gap: 12 }}>
                  <View style={{ flex: 1, gap: 4 }}>
                    <Text style={{ ...theme.typography.cardTitle, color: theme.colors.foreground }}>{item.title}</Text>
                    <Text style={{ ...theme.typography.body, color: theme.colors.muted }}>{item.business_profile?.brand_name ?? "Brand"}</Text>
                  </View>
