...
function HomeTierHero({ earnings, loading }: { earnings?: BrandHomeEarnings; loading: boolean }) {
...
function routeToCampaignOrList(campaigns: CampaignListItem[]) {
  ...
  router.push('/(app)/(brand-tabs)/campaigns');
}
...
export default function BrandHomeScreen() {
  ...
  const brandEarnings = useMemo<BrandHomeEarnings>(
    () => ({
  ...
  const heroLoading = profileLoading || campaignsLoading;
  ...
  return (
    ...
    >
      <HomeTierHero earnings={brandEarnings} loading={heroLoading} />
