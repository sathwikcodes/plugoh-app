...
export interface DataStore {
  ...
  update<T extends Row>(table: string, options: QueryOptions, values: Row, select?: string): Promise<T[]>;
  upsert<T extends Row>(table: string, values: Row, onConflict?: string, select?: string): Promise<T>;
}
...
export class SupabaseDataStore implements DataStore {
  ...
  async list<T extends Row>(table: string, options: QueryOptions = {}, select = "*") {
    const { data, error } = await this.applyOptions(this.client.from(table).select(select), options);
  ...
  async update<T extends Row>(table: string, options: QueryOptions, values: Row, select = "*") {
    const query = this.applyFilters(this.client.from(table).update(values).select(select), options);
  ...
  async upsert<T extends Row>(table: string, values: Row, onConflict?: string, select = "*") {
    const { data, error } = await this.client.from(table).upsert(values, { onConflict }).select(select).single();
    if (error) throw error;
    return data as unknown as T;
  }

  private applyOptions(query: any, options: QueryOptions) {
    let next = this.applyFilters(query, options);
  ...
  private applyFilters(query: any, options: QueryOptions) {
    ...
    for (const [key, value] of Object.entries(options.contains ?? {})) next = next.contains(key, value);
