...
function makeApp(seed: Record<string, unknown> = {}, overrides?: { withStorage?: boolean; pushShouldFail?: boolean }) {
...
describe("Foundation Hardening Routes", () => {
  it("returns /me/bootstrap for authenticated users", async () => {
    const { app } = makeApp();
    const res = await app.request("/me/bootstrap", { headers: { authorization: "Bearer influencer" } });
  ...
  it("validates /influencer/onboarding payload and persists onboarding basics", async () => {
    ...
    const valid = await app.request("/influencer/onboarding", {
      method: "POST",
      headers: { authorization: "Bearer influencer", "content-type": "application/json" },
      body: JSON.stringify({ full_name: "Creator New", phone: "+919999999999", location: "Hyderabad" }),
    });
    expect(valid.status).toBe(200);
    const profile = store.tables.get("influencer_profiles")?.find((row) => row.user_id === influencerId);
  ...
  it("supports /push/register and /push/unregister with auth + validation", async () => {
    ...
    expect(store.tables.get("user_push_tokens")?.find((row) => row.user_id === influencerId)?.expo_push_token).toBe(
    ...
    expect(store.tables.get("user_push_tokens")?.find((row) => row.user_id === influencerId)?.expo_push_token).toBeNull();
  });
  ...
  it("enforces /campaigns/:id/messages/attachment validation and storage provider checks", async () => {
    ...
    const missingFile = await withStorage.app.request(`/campaigns/${campaignId}/messages/attachment`, {
      method: "POST",
      headers: { authorization: "Bearer influencer" },
      body: new FormData(),
    });
    ...
    const noStorage = await withoutStorage.app.request(`/campaigns/${campaignId}/messages/attachment`, {
      method: "POST",
      headers: { authorization: "Bearer influencer" },
      body: form,
    });
  ...
  it("validates mobile Instagram OAuth state and redirects correctly for valid callback", async () => {
    ...
    const callback = await app.request(`/auth/callback/instagram?code=test-code&state=${encodeURIComponent(state)}`, {
      headers: { cookie: `ig_oauth_state=${encodeURIComponent(state)}` },
    });
    expect(callback.status).toBe(302);
    expect(callback.headers.get("location")).toContain("plugoh://instagram/callback?status=success&role=influencer&source=onboarding");
  });
  ...
  it("keeps request success path when push provider fails while still attempting push delivery", async () => {
    const { app, store, push } = makeApp(
      {
        user_push_tokens: [{ id: "push-1", user_id: influencerId, expo_push_token: "ExponentPushToken[abc]", platform: "ios" }],
      },
      { pushShouldFail: true },
    );
