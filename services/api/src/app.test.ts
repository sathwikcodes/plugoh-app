...
import { FakeAiProvider, FakeEmailProvider, FakeInstagramProvider, FakePaymentProvider, FakeStorageProvider } from "./testing/fakes.js";
...
function makeApp(seed = {}) {
  const store = new MemoryDataStore({
    ...
    business_profiles: [
      { id: "bp-1", user_id: businessId, brand_name: "Plugoh Cafe", brand_type: "Restaurant/Cafe", brand_location: "Hyderabad" },
    ],
    influencer_profiles: [
      {
...
describe("Plugoh API", () => {
  ...
  it("does not run auth verification for public influencer discovery", async () => {
    const store = new MemoryDataStore({
      influencer_profiles: [{ id: influencerProfileId, user_id: influencerId, display_name: "Creator One", is_active: true }],
    });
    let authCalls = 0;
    const app = createApp({
      store,
    ...
    const res = await app.request("/influencers", { headers: { authorization: "Bearer should-not-be-read" } });
    expect(res.status).toBe(200);
  ...
  it("accepts a requested campaign into payment_pending", async () => {
    const { app, store } = makeApp({
      campaigns: [{ id: campaignId, business_id: businessId, influencer_id: influencerId, title: "Booking", status: "requested", price_offered: 10000, platform_fee_amount: 1200, total_charged_amount: 11200 }],
    });
  ...
  it("verifies escrow payment idempotently", async () => {
    ...
    const { app, store } = makeApp({
      campaigns: [{ id: campaignId, business_id: businessId, influencer_id: influencerId, title: "Booking", status: "payment_pending", price_offered: 10000, platform_fee_amount: 1200, total_charged_amount: 11200, razorpay_order_id: orderId }],
    });
    const payload = {
  ...
  it("submits and approves delivery", async () => {
    ...
    const approve = await app.request(`/campaigns/${campaignId}/approve`, { method: "POST", headers: { authorization: "Bearer business" } });
  ...
  it("rejects duplicate delivery submissions", async () => {
    const { app } = makeApp({
      campaigns: [{ id: campaignId, business_id: businessId, influencer_id: influencerId, title: "Booking", status: "in_escrow", price_offered: 10000, platform_fee_amount: 1200, total_charged_amount: 11200 }],
      deliveries: [{ id: "delivery-1", campaign_id: campaignId, submitted_by: influencerId, content_url: "path/file.mp4" }],
    });
  ...
  it("release escrow is idempotent after completion", async () => {
    const { app, store } = makeApp({
      campaigns: [{ id: campaignId, business_id: businessId, influencer_id: influencerId, title: "Booking", status: "completed", price_offered: 10000, platform_fee_amount: 1200, total_charged_amount: 11200 }],
      escrow_transactions: [
        { id: "tx-1", campaign_id: campaignId, type: "payout_influencer", amount_paise: 1000000, status: "pending" },
        { id: "tx-2", campaign_id: campaignId, type: "platform_fee", amount_paise: 120000, status: "success" },
      ],
    });
  ...
  it("validates Razorpay webhook signatures", async () => {
    const { app, store } = makeApp({
      campaigns: [{ id: campaignId, business_id: businessId, influencer_id: influencerId, status: "payment_pending", razorpay_order_id: "order_failed", payment_status: "authorized" }],
    });
    const body = JSON.stringify({ event: "payment.failed", payload: { payment: { entity: { order_id: "order_failed" } } } });
