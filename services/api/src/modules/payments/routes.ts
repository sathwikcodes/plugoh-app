...
export function paymentRoutes(deps: RouteDeps) {
  ...
  app.post(
    ...
    zJson(createEscrowOrderSchema),
    async (c) => {
      return ok(
        c,
        await deps.services.payments.createEscrowOrder(
          deps.requireUser(c),
  ...
  app.post(
    '/payment/verify-booking-payment',
    auth,
    deps.paymentRateLimit,
    requireRole('business'),
    zJson(verifyBookingPaymentSchema),
    async (c) => {
      const cached = await deps.claimIdempotency(deps.store, c.req.header('idempotency-key'));
      if (cached) return ok(c, cached);
      const result = await deps.services.payments.verifyBookingPayment(
        deps.requireUser(c),
        c.req.valid('json'),
      );
      await deps.storeIdempotency(deps.store, c.req.header('idempotency-key'), result);
  ...
  app.post(
    '/payment/capture-booking-payment',
    requireSecret('x-internal-secret', deps.config.internalSecret, 'internal secret'),
    zJson(campaignIdSchema),
    async (c) => {
      return ok(
        c,
        await deps.services.payments.captureBookingPayment(c.req.valid('json').campaign_id),
      );
    },
  );
  app.post('/payment/release-escrow', deps.paymentRateLimit, zJson(campaignIdSchema), async (c) => {
    ...
    const result = await deps.services.payments.releaseEscrow(
      user,
      c.req.valid('json').campaign_id,
    );
    await deps.storeIdempotency(deps.store, c.req.header('idempotency-key'), result);
