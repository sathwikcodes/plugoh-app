...
export function messagingRoutes(deps: RouteDeps) {
    ...
    async (c) => {
      ...
      return ok(
        ...
        await deps
          ...
          .messaging.messages(deps.requireUser(c), c.req.valid('param').id, {
  ...
  app.post(
    '/campaigns/:id/messages/attachment',
    auth,
    deps.authDefaultRateLimit,
    zParam(idParamSchema),
    async (c) => {
  ...
  app.post(
    ...
    auth,
    deps.authDefaultRateLimit,
    zJson(requestCallSchema),
    async (c) =>
      ok(
        c,
        await deps.services.messaging.requestCall(
          deps.requireUser(c),
          c.req.valid('json').campaignId,
        ),
      ),
  );
