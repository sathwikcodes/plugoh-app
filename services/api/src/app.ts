...
export function createApp(options: AppOptions = {}) {
  ...
  app.get("/influencer/profile", requireAuth(store, authVerifier), requireRole("influencer"), async (c) => {
    return ok(c, await services.profiles.getInfluencer(requireUser(c)));
  });
  app.patch("/influencer/profile", requireAuth(store, authVerifier), requireRole("influencer"), zJson(influencerProfilePatchSchema), async (c) => {
    return ok(c, await services.profiles.updateInfluencer(requireUser(c), c.req.valid("json")));
  });
  ...
  app.post("/campaigns/:id/dispute", requireAuth(store, authVerifier), requireRole("business"), zParam(idParamSchema), zJson(disputeSchema), async (c) => {
    return ok(c, await services.campaigns.dispute(requireUser(c), c.req.valid("param").id, c.req.valid("json").reason));
  });
  app.post("/campaigns/:id/deliver", requireAuth(store, authVerifier), requireRole("influencer"), zParam(idParamSchema), zJson(deliverySubmitSchema), async (c) => {
    return ok(c, await services.delivery.submit(requireUser(c), c.req.valid("param").id, c.req.valid("json")));
  });
  app.get("/campaigns/:id/delivery/url", requireAuth(store, authVerifier), requireRole("business"), zParam(idParamSchema), async (c) => {
    return ok(c, await services.delivery.signedUrl(requireUser(c), c.req.valid("param").id));
  });
  ...
  app.post("/payment/create-escrow-order", requireAuth(store, authVerifier), requireRole("business"), zJson(createEscrowOrderSchema), async (c) => {
    return ok(c, await services.payments.createEscrowOrder(requireUser(c), c.req.valid("json").campaign_id));
  });
  // Spec-defined alias for the same standard escrow-order flow. Keep behavior identical.
  ...
  app.post("/payment/verify-escrow", requireAuth(store, authVerifier), requireRole("business"), zJson(verifyEscrowSchema), async (c) => {
    return ok(c, await services.payments.verifyEscrow(requireUser(c), c.req.valid("json")));
  });
  // Spec-defined alias for the same standard escrow verification flow. Keep behavior identical.
  ...
  app.post("/payment/verify-booking-payment", requireAuth(store, authVerifier), requireRole("business"), zJson(verifyBookingPaymentSchema), async (c) => {
    return ok(c, await services.payments.verifyBookingPayment(requireUser(c), c.req.valid("json")));
  });
  ...
  app.post("/delivery/upload", requireAuth(store, authVerifier), requireRole("influencer"), bodyLimit({ maxSize: 50 * 1024 * 1024 }), async (c) => {
    ...
    return ok(c, await services.delivery.upload(requireUser(c), campaignId, file));
  });

  app.get("/inbox/influencer", requireAuth(store, authVerifier), requireRole("influencer"), async (c) => ok(c, await services.messaging.inbox(requireUser(c), "influencer")));
  app.get("/inbox/business", requireAuth(store, authVerifier), requireRole("business"), async (c) => ok(c, await services.messaging.inbox(requireUser(c), "business")));
  app.get("/campaigns/:id/messages", requireAuth(store, authVerifier), zParam(idParamSchema), async (c) => ok(c, await services.messaging.messages(requireUser(c), c.req.valid("param").id)));
  app.post("/campaigns/:id/messages", requireAuth(store, authVerifier), zParam(idParamSchema), zJson(messageCreateSchema), async (c) => {
    return created(c, await services.messaging.send(requireUser(c), c.req.valid("param").id, c.req.valid("json")));
  });
  app.patch("/campaigns/:id/messages/read", requireAuth(store, authVerifier), zParam(idParamSchema), async (c) => ok(c, await services.messaging.markRead(requireUser(c), c.req.valid("param").id)));
  app.post("/inbox/request-call", requireAuth(store, authVerifier), zJson(requestCallSchema), async (c) => ok(c, await services.messaging.requestCall(requireUser(c), c.req.valid("json").campaignId)));

  app.get("/notifications", requireAuth(store, authVerifier), async (c) => ok(c, await services.notifications.list(requireUser(c))));
  app.patch("/notifications/read", requireAuth(store, authVerifier), zJson(notificationsReadSchema), async (c) => ok(c, await services.notifications.markRead(requireUser(c), c.req.valid("json"))));
  ...
  app.get("/instagram/connect", requireAuth(store, authVerifier), zQuery(instagramConnectQuerySchema), async (c) => {
    ...
    const result = services.instagram.connect(input);
  ...
  app.post("/instagram/sync", requireAuth(store, authVerifier), requireRole("influencer"), async (c) => ok(c, await services.instagram.sync(requireUser(c))));
...
function requireRoleValue(c: Context<AppEnv>) {
